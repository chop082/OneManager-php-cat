<?php
// Local storage driver for OneManager
// Put this file in disk/LocalStorage.php

class LocalStorage {
    protected $disktag;
    protected $rootPath;   // 本地存储根目录
    protected $fileTypes;  // 允许的文件类型
    public $error;
    ...
    function __construct($tag) {
        ...
        if ($this->rootPath == '') {
            $this->error = ['stat' => 500, 'body' => 'Root path not set'];
            return;
        }
        ...
        if (!$this->rootPath || !is_dir($this->rootPath)) {
            $this->error = ['stat' => 500, 'body' => 'Invalid root path'];
            return;
        }

        // 检查目录权限
        if (!is_readable($this->rootPath) || !is_writable($this->rootPath)) {
            $this->error = ['stat' => 500, 'body' => 'Directory permission denied'];
            return;
        }
    ...
    public function list_files($path = '/') {
        ...
        if (!$this->checkPath($physicalPath)) {
            return ['error' => ['code' => 'InvalidPath', 'message' => 'Path access denied']];
        }
        ...
        if (!is_dir($physicalPath)) {
            ...
            return ['error' => ['code' => 'NotFound', 'message' => 'Path not found']];
        }
        ...
        foreach ($items as $item) {
            ...
            if ($this->checkPath($physicalPath . '/' . $item)) {
                $list[$item] = $this->getFileInfo($physicalPath . '/' . $item, $path . '/' . $item);
            }
        }
    ...
    public function Rename($file, $newname) {
        ...
        return ['error' => ['code' => 'RenameFailed', 'message' => 'Failed to rename']];
    }
    ...
    public function Delete($file) {
        ...
        if (is_dir($path)) {
            if ($this->deleteDir($path)) {
                return ['stat' => 200, 'body' => 'Delete success'];
            }
        } else {
            if (unlink($path)) {
                return ['stat' => 200, 'body' => 'Delete success'];
        ...
        return ['error' => ['code' => 'DeleteFailed', 'message' => 'Failed to delete']];
    }
    ...
    public function Move($file, $folder) {
        ...
        return ['error' => ['code' => 'MoveFailed', 'message' => 'Failed to move']];
    }

    // 复制文件/文件夹
    public function Copy($file) {
        $sourcePath = $this->getPhysicalPath($file['path'] . '/' . $file['name']);
        $targetPath = $this->getPhysicalPath($file['path'] . '/Copy of ' . $file['name']);
        ...
        if (is_dir($sourcePath)) {
            if ($this->copyDir($sourcePath, $targetPath)) {
                return $this->files_format($this->getFileInfo($targetPath, $file['path'] . '/Copy of ' . $file['name']));
            }
        } else {
        ...
        return ['error' => ['code' => 'CopyFailed', 'message' => 'Failed to copy']];
    }
    ...
    public function Create($parent, $type, $name, $content = '') {
        ...
        return ['error' => ['code' => 'CreateFailed', 'message' => 'Failed to create']];
    }
    ...
    public function Upload($path, $tmpfile) {
        ...
        if (!in_array($ext, $this->fileTypes)) {
            return ['error' => ['code' => 'InvalidType', 'message' => 'File type not allowed']];
        }

        // 检查文件大小
        $maxSize = getConfig('max_size', $this->disktag) ?: 10 * 1024 * 1024; // 默认10MB
        if ($tmpfile['size'] > $maxSize) {
            return ['error' => ['code' => 'FileTooLarge', 'message' => 'File too large']];
        }

        if (move_uploaded_file($tmpfile['tmp_name'], $targetPath)) {
            return $this->files_format($this->getFileInfo($targetPath, $path . '/' . $filename));
        }
        return ['error' => ['code' => 'UploadFailed', 'message' => 'Failed to upload']];
    }
    ...
    public function AddDisk() {
        ...
        if (isset($_GET['install0'])) {
            ...
            $tmp['allowed_types'] = implode(',', $this->fileTypes);

            $response = setConfigResponse(setConfig($tmp, $this->disktag));
            return message($response, getconstStr('Success'), 201);
        }

        $html = '<form id="form1" name="form1" action="" method="post" onsubmit="return checkPath(this);">
            <label>' . getconstStr('DiskName') . ':</label><br>
            <input type="text" name="diskname" placeholder="' . getconstStr('DiskName') . '"><br>
            <label>Root Path:</label><br>
        ...
        return message($html, getconstStr('SetStorage'), 201);
    }
    ...
    protected function checkPath($path) {
        ...
        if ($realPath === false) return true; // 允许创建新路径
    ...
    public function Edit($file, $content) {
        ...
        if (file_put_contents($path, $content) !== false) {
            return ['stat' => 200, 'body' => 'Edit success'];
        }
        return ['error' => ['code' => 'EditFailed', 'message' => 'Failed to edit file']];
    }

    // 支持加密功能
    public function Encrypt($folder, $passfile, $pass) {
        $path = $this->getPhysicalPath($folder['path'] . '/' . $passfile);
        if (!$this->checkPath($path)) {
            return ['error' => ['code' => 'InvalidPath', 'message' => 'Path access denied']];
        }
        ...
        if ($pass === '') {
            ...
            if (file_exists($path) && unlink($path)) {
                return ['stat' => 200, 'body' => 'Remove password success'];
            }
        } else {
            // 创建或更新密码文件
            if (file_put_contents($path, $pass) !== false) {
                return ['stat' => 200, 'body' => 'Set password success'];
            }
        }
        return ['error' => ['code' => 'EncryptFailed', 'message' => 'Failed to set password']];
    }
    ...
    public function bigfileupload($path) {
        if (!isset($_POST['uploadid'])) {
            ...
            if (isset($_POST['complete'])) {
                ...
                return ['error' => ['code' => 'UploadFailed', 'message' => 'Failed to complete upload']];
            }
